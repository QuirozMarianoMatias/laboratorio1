...
void mostrarListadoAlumnos(eAlumno listadoDeAlumnos[],eAlumno listadoLocalidad[], int tam)
{
    ...
    for(i=0; i<tam; i++)
    {
        if(listadoDeAlumnos[i].estado==OCUPADO)
        {
            localAux = buscarLocalidad(listadoDeAlumnos,listadoLocalidad,3,listadoDeAlumnos[i].idLocalidad);
            mostrarUnAlumno(listadoDeAlumnos,listadoLocalidad);
        }
...
{
    printf("%4d %25s %8.2f %10s\n",miAlumno.legajo,miAlumno.nombre, miAlumno.promedio, unaLocalidad.localidad);
}
...
int buscarLocalidad(eAlumno listaDeAlumnos [],eLocalidad listaDeLocalidades [], int cant, int id)
{
    ...
    int index = -1;
     ...
     for(i=0; i<cant; i++)
    {
        if(id==listaDeLocalidades[i].idLocalidad && listaDeAlumnos[i].estado==OCUPADO)
        {
            index = i;
            break;
        }
